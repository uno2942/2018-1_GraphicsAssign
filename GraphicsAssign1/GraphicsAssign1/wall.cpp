...
GameObjectTree* GetLeftWall() {
	...
	if (LeftwallTree == NULL)
	{
		...
		GameObject* temp = new GameObject(new Box("leftwall", WORLD_COORD_MAP_YLEN, WORLD_COORD_MAP_ZLEN, 0, MY_WALL_COLOR));
		temp->object->SetPosition(0, WORLD_COORD_MAP_ZLEN / 2, WORLD_COORD_MAP_YLEN / 2);
		temp->object->rotationAxis = Vector3(0, 1, 0);
		temp->object->rotation = -3.141592653 / 2;
...
GameObjectTree* GetRightWall() {
	...
	if (RightwallTree == NULL)
	{
		...
		GameObject* temp = new GameObject(new Box("rightwall", WORLD_COORD_MAP_YLEN, WORLD_COORD_MAP_ZLEN, 0, MY_WALL_COLOR));
...
GameObjectTree* GetFrontWall() {
	...
	if (FrontwallTree == NULL)
	{
		...
		GameObject* temp = new GameObject(new Box("frontwall", WORLD_COORD_MAP_XLEN, WORLD_COORD_MAP_YLEN, 0, MY_WALL_COLOR));
		temp->object->SetPosition(WORLD_COORD_MAP_XLEN / 2, 0, WORLD_COORD_MAP_YLEN / 2);
		temp->object->rotationAxis = Vector3(1, 0, 0);
		temp->object->rotation = 3.141592653 / 2;
...
GameObjectTree* GetBackWall() {
	...
	if (BackwallTree == NULL)
	{
		...
		GameObject* temp = new GameObject(new Box("backwall", WORLD_COORD_MAP_XLEN, WORLD_COORD_MAP_YLEN, 0, MY_WALL_COLOR));
